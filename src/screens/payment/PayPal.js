...
export class PayPal extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      productsMoney: Object.keys(
        this.props.navigation.getParam('CartData').cartInfo
      ).length
        ? this.props.navigation
            .getParam('CartData')
            .cartInfo.products.filter(p => p.methodMoney === 'buyOfMoney')
        : null,
      productsBonus: Object.keys(
        this.props.navigation.getParam('CartData').cartInfo
      ).length
        ? this.props.navigation
            .getParam('CartData')
            .cartInfo.products.filter(p => p.methodMoney === 'buyOfPoints')
        : null,

      // DELIVERY
      deliveryID: this.props.navigation.getParam('CartData').deliveryData
        .selected,
      deliveryName: this.props.navigation.getParam('CartData').deliveryData
        .name,
  ...
  componentWillMount() {
    const seller_username =
      'AcIGgF0XrYF4her0z9zSlXCuUnqEDazkaM0DDU5emhhp70UggARzDbd5LW1yQhr8qEaV2Q7r-AmK3bHH'
    const seller_password =
      'EHN2OcacoZFk8823hZ_oWXHoI_T-SmtDcs2XklW07F0Lwd57Tjjs9C63hdz_5woXimmEenYveCY2zMtF'
    const token = base64.encode(`${seller_username}:${seller_password}`)
    const orderPrice = Object.keys(this.state.cart.cartInfo).length
      ? this.state.cart.cartInfo.discountValue
      : this.state.user.selectedUserType === 'EK'
      ? this.state.cart.SofortKaufen.price
      : this.state.cart.SofortKaufen.companyPrice
    ...
    try {
      fetch('https://api.paypal.com/v1/oauth2/token', {
            ...
            transactions: [
              {
                amount: {
                  ...
                  currency: 'EUR',
                  details: {
                    subtotal: orderPrice.toFixed(2),
                    shipping: deliveryPrice.toFixed(2)
                  }
                },
                description:
                  this.state.paymentName === 'PayPal'
                    ? 'Teleropa PayPal payment.'
                    : 'Teleropa PayPal Kauf auf Rechnung payment.',
                item_list: {
                  items: Object.keys(this.state.cart.cartInfo).length
          ...
          fetch('https://api.paypal.com/v1/payments/payment', {
            ...
            .then(json => {
              ...
              if (typeof name !== 'undefined' && name === 'VALIDATION_ERROR') {
                ...
                Alert.alert(
                  `PayPal Zahlungsvalidierungsfehler!`,
                  `${details[0].issue}`,
                ...
                if (this.state.paymentName === 'Rechnung') {
                  if (approvalUrl.length != 0) {
                    ...
                    this.setState({
                      paymentId: id,
                      approvalUrl:
                        'https://www.paypal.com/webapps/xoonboarding?token=' +
                        rech_token
                    })
                  } else {
                    ...
                    Alert.alert(
                      'WARNUNG!',
                      'Bei der Anmeldung ist ein Fehler auf dem Paypal-Server entstanden',
            ...
            .catch(err => {
              ...
              Alert.alert(
                'WARNUNG!',
                'Die Anwendung hat bei der Anfrage-Bearbeitung einen Fehler ausgelöst: ' +
                  err,
        ...
        .catch(err => {
          ...
          Alert.alert(
            'WARNUNG!',
            'Paypal hat beim Versuch, sich anzumelden, einen Fehler verursacht',
  ...
  WebViewChange = WebViewState => {
    ...
    console.log(
      '<onNavigationStateChange>...............WebViewState.url.includes'
    )
    if (p >= 0 && this.state.count) {
      // if (p >= 0) {
      console.log(
        '<onNavigationStateChange>...............WebViewState.url.includes  P>=0'
      )
      ...
      fetch(
        ...
        .then(json => {
          ...
          if (json === undefined) {
            ...
            Alert.alert(
              'WARNUNG!',
              'Ihre Zahlung wird nicht bestätigt',
          ...
          if (json.failed_transactions.length == 0) {
            //---------------------------------------------------------------------------
            Object.keys(this.props.navigation.getParam('CartData').cartInfo)
              .length
            ...
            Alert.alert(
              'Auftragsabwicklung!!',
              `PayPal-Bestellung erfolgreich abgeschlossen\n\r${prepayment_message}`,
              // '','***********************',
              [
                {
                  text: 'Ja',
