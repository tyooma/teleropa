...
class Main extends Component {
    ...
    headerLeft: (
      <>
        ...
        <Image
          ...
          source={require("../../assets/teleropa-logo.png")}
          key={"menuTeleropaLogo"}
        />
      </>
    ),
    headerRight: (
  ...
  getCategoriesCards() {
    return this.props.mainPage.categories.map(
      ({ categoryName, categoryImageURL, categoryID }) => {
        if (categoryImageURL)
          return (
  ...
  getNewest() {
    return this.props.mainPage.newest.products.filter(
      ({ productID, productName, previewImgURL, price, companyPrice }) => {
        if (previewImgURL)
          return (
            <NewestListItem
              key={productID}
  ...
  getOffers() {
    return this.props.mainPage.offers.products.filter(
      ({
        productID,
        productName,
        previewImgURL,
    ...
    return (
      <View style={{ flex: 1 }}>
        <ScrollView>
          ...
          >
            <View style={styles.topContainer}>
              ...
              <Text style={styles.topText}>
                Kostenloser Hin- und Rückversand ab 40 EUR*
              </Text>
              <Image source={image} style={styles.topImage} />
          ...
          <Text style={styles.helperText}>Unsere Kategorien</Text>
          <View
            style={{ flexDirection: "row", marginLeft: 18, flexWrap: "wrap" }}
          >
            {this.getCategoriesCards()}
          </View>

          <Text style={styles.helperText}>Aktuelle Angebote</Text>
              ...
              renderItem={({ item }) => {
                return (
                  <OffersListItem
                    ...
                    image={{ uri: item.previewImgURL }}
                    price={item.price}
                    companyPrice={item.companyPrice}
                    salePrice={item.pseudoprice}
                  />
                );
              }}
          ...
          <Text style={styles.helperText}>Unsere Neuheiten</Text>
          ...
          <Text style={styles.helperText}>Teleropa Service</Text>
          <View style={{ flexDirection: "row", height: 170, flexWrap: "wrap" }}>
            <FlatList
              horizontal
