      ...
      return (
        <View style={styles.searchStory}>
          ...
          {this.state.searchStory.map((searchItem, index) => (
            <SearchStoryItem
              onPress={() => this.onSearchChange(searchItem)}
              text={searchItem}
              key={index}
            />
          ))}
      ...
      switch (sortBy) {
        ...
        case "price_up":
          sorted = this.state.products.sort((first, second) =>
            parseFloat(first.companyPrice) < parseFloat(second.companyPrice)
              ? -1
              : parseFloat(second.pcompanyPricerice) <
          ...
          <FlatList
              ...
              return (
                <View style={{ paddingBottom: 8 }}>
                  <ProductListItem
                    ...
                    salePercent={
                      productSalePercent ? productSalePercent.int : null
                    }
            ...
            ListHeaderComponent={
              <FilterButton
                ...
                toPrice={toPrice}
                sortBy={sortBy}
                screenBack={this.props.navigation.state.routeName}
              />
            }
            ListFooterComponent={
